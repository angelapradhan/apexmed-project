    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-100">
      ...
      <nav className="flex items-center justify-between px-6 md:px-20 py-6 bg-white sticky top-0 z-50">
        ...
        <div className="hidden lg:flex items-center gap-8 text-[15px] font-semibold text-slate-500 relative">
          ...
          <button onClick={scrollToServices} className="hover:text-blue-500 transition-colors">Services</button>
          <button onClick={scrollToDoctors} className="hover:text-blue-500 transition-colors">Doctors</button>
          <button onClick={scrollToContact} className="hover:text-blue-500 transition-colors">Contact</button>
        </div>
      ...
      <header className="px-6 md:px-20 pt-8 pb-12 relative overflow-hidden bg-white mb-12">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center">
          <div className="md:w-1/2 space-y-7 z-10 text-center md:text-left">
            ...
            <h1 className="text-6xl md:text-7xl font-extrabold text-[#2d3e50] leading-[1.1]">Healthcare <br /> Solutions</h1>
            ...
            <button 
              onClick={() => navigate('/login')}
              className="px-10 py-4 bg-blue-500 text-white rounded-full font-bold shadow-xl shadow-blue-200 hover:-translate-y-1 transition-transform"
            >
              Find Doctors
            </button>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-20">
            ...
            <div className="lg:w-1/2 space-y-8">
              ...
              <p className="text-slate-400 leading-relaxed text-lg max-w-lg">
                We provide comprehensive dental care, from routine checkups to advanced treatments.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-blue-500 rounded-[50px] p-10 md:p-16 flex flex-col md:flex-row items-center justify-between text-white relative overflow-hidden shadow-2xl">
             <div className="text-left space-y-6 md:w-1/2 z-10">
                ...
                <p className="text-blue-100 text-lg opacity-90 max-w-sm">Schedule consultations in just a few clicks.</p>
                <button 
                  onClick={() => navigate('/login')}
                  className="px-8 py-3 bg-white text-blue-500 rounded-full font-bold hover:shadow-lg transition-all"
                >
                  Learn more
                </button>
             </div>
             <div className="md:w-1/2 flex justify-end mt-12 md:mt-0"><Stethoscope size={240} className="text-white/20 rotate-30" /></div>
          </div>
        </div>
      </section>
...
const ActionCard = ({ title, icon, description, btn, onClick }) => (
  <div className="bg-white p-8 rounded-[40px] shadow-xl shadow-slate-100 border border-slate-50 flex flex-col items-center text-center group hover:-translate-y-2 transition-transform">
    ...
    <button onClick={onClick} className="w-full py-2.5 bg-[#f0f7ff] text-blue-500 rounded-full text-xs font-bold hover:bg-blue-500 hover:text-white transition-all">{btn}</button>
  </div>
);
